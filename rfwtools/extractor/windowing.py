...
::
    ...
    window_extractor(ds.example_set.loc[0, 'example], windows={'my_window': -500}, window_size=100)
    ...
    window_extractor(ds.example_set.loc[0, 'example], windows={'stable': -1536, 'impending': -105}, window_size=500)

    # Run this on every example in the example set and produce a corresponding feature set for pre-fault signal data.
    ds.produce_feature_set(window_extractor, windows={'my_window': -500}, window_size=100)
...
def window_extractor(example: Example, windows: Dict[str, float], window_size: int, signals: List[str] = None,
    ...
    return _window_extractor(example=example, windows=windows, window_size=window_size, signals=signals,
                             standardize=standardize, downsample=downsample, ds_kwargs=ds_kwargs, query=query)


def window_extractor_faulted_cavity(example: Example, windows: Dict[str, float], window_size: int, waveforms: List[str],
    ...
    # Call the real workhorse
    out = _window_extractor(example=example, windows=windows, window_size=window_size, signals=signals,
                            standardize=standardize, downsample=downsample, ds_kwargs=ds_kwargs, query=query)
...
def _window_extractor(example: Example, windows: Dict[str, float], window_size: int, signals: List[str],
                      standardize: bool = True, downsample: bool = False, ds_kwargs: dict = {'num': 256, 'axis': 0},
                      query: str = None) -> pd.DataFrame:
    """
    Extract labeled time windows of a Example's event_df.  Will produce one feature row for each key in windows.
